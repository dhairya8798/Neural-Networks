...
class CNN(object):
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        """
         ...
         :return: None
         """
        layer = layers.Dense(units=num_nodes, activation=activation, trainable=trainable, name=name)
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        """
         ...
         :return: Layer object
         """
        layer = layers.Conv2D(num_of_filters, kernel_size, padding=padding, strides=strides, activation=activation, name=name, trainable=trainable)
        self.model.add(layer)
        return layer
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        """
         ...
         :return: Layer object
         """
        layer = layers.MaxPool2D(pool_size=pool_size, padding=padding, strides=strides, name=name)
        self.model.add(layer)
        return layer
    ...
    def set_training_flag(self,layer_numbers=[],layer_names="",trainable_flag=True):
        ...
        if isinstance(layer_numbers, list):
            for i in layer_numbers:
                self.model.get_layer(index = i).trainable = trainable_flag
        else:
            self.model.get_layer(index = layer_numbers).trainable = trainable_flag

        if isinstance(layer_names, list):
            for j in layer_names:
                self.model.get_layer(name = j).trainable = trainable_flag
        else:
    ...
    def get_weights_without_biases(self,layer_number=None,layer_name=""):
        ...
        if layer_number is None:
            if layer_name == 'input':
                ...
                weight_matrix = self.model.get_layer(name = layer_name).get_weights()

        else:
            if layer_number == 0:
                return None
            elif layer_number > 0:
                weight_matrix = self.model.get_layer(index = layer_number - 1).get_weights()
            else:
                weight_matrix = self.model.get_layer(index = layer_number).get_weights()
    ...
    def get_biases(self,layer_number=None,layer_name=""):
        ...
        if layer_number is not None:
            if layer_number == 0:
                ...
                biases = self.model.get_layer(index = layer_number).get_weights()
        else:
            if layer_name == "input":
                return None
            else:
    ...
    def set_weights_without_biases(self,weights,layer_number=None,layer_name=""):
        ...
        This means that the first layer with activation function is layer zero
         ...
         :return: None
         """
        weights_to_assign = list()
        weights_to_assign.append(np.array(weights))

        if layer_number is None:
            if layer_name == 'input':
                return None
            else:
                weights_to_assign.append(self.model.get_layer(name = layer_name).get_weights()[1])
                self.model.get_layer(name = layer_name).set_weights(weights_to_assign)
        else:
            if layer_number == 0:
                return None
            elif layer_number > 0:
                weights_to_assign.append(self.model.get_layer(index = layer_number - 1).get_weights()[1])
                self.model.get_layer(index = layer_number - 1).set_weights(weights_to_assign)
            else:
                weights_to_assign.append(self.model.get_layer(index = layer_number).get_weights()[1])
                self.model.get_layer(index = layer_number).set_weights(weights_to_assign)
    ...
    def set_biases(self,biases,layer_number=None,layer_name=""):
        ...
        :return: none
        """
        biases_to_assign = list()

        if layer_number is None:
            if layer_name == 'input':
                return None
            else:
                biases_to_assign.append(self.model.get_layer(name = layer_name).get_weights()[0])
                biases_to_assign.append(np.array(biases))
                self.model.get_layer(name = layer_name).set_weights(biases_to_assign)
        else:
            if layer_number == 0:
                return None
            elif layer_number > 0:
                biases_to_assign.append(self.model.get_layer(index = layer_number - 1).get_weights()[0])
                biases_to_assign.append(np.array(biases))
                self.model.get_layer(index = layer_number - 1).set_weights(biases_to_assign)
            else:
                biases_to_assign.append(self.model.get_layer(index = layer_number).get_weights()[0])
                biases_to_assign.append(np.array(biases))
                self.model.get_layer(index = layer_number).set_weights(biases_to_assign)
    ...
    def remove_last_layer(self):
        ...
        removed_last_layer = self.model.pop()
        # print("----------", self.model.summary())
        self.model = keras.Sequential(self.model)
        return removed_last_layer
    ...
    def load_a_model(self,model_name="",model_file_name=""):
        ...
        if model_name.lower() == "vgg16":
            ...
            self.model = keras.Sequential(load_model(model_file_name).layers)
    ...
    def evaluate(self,X,y):
        """
         ...
         """
        loss_value, metric_value = self.model.evaluate(X, y)
